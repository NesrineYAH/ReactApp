...
const [typingMessage, setTypingMessage] = useState("");
 ...
 // Effet pour le typing quand le chat s'ouvre
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      ...
      const interval = setInterval(() => {
        ...
        if (i === fullMessage.length) {
          ...
          // Ajouter le message final dans messages
          setMessages([{ from: "bot", content: fullMessage }]);
          setTypingMessage("");
        }
      }, 100); // vitesse du typing en ms
...
//         onClick={() => setIsOpen(true)}
          onClick={handleOpen}   // <- ici on appelle la nouvelle fonction
        >
          <svg
            width="20"
            height="20"
      ...
      {isOpen && (
        <div className="chatbox-container open">
          ...
          {/* <ChatBox messages={messages} setMessages={setMessages} typingMessage={typingMessage}/> */}
        <ChatBox
  messages={messages}
